...
import com.myvideoyun.shader.MVYMagicShader;
...
public class MVYGPUImageShortVideoFilter extends MVYGPUImageFilter {

    public enum MVY_VIDEO_EFFECT_TYPE {
        MVY_VIDEO_EFFECT_NONE(0),                //0 基本特效
        MVY_VIDEO_EFFECT_SPIRIT_FREED(1),        //1 灵魂出窍
        MVY_VIDEO_EFFECT_SHAKE(2),               //2 抖动
        MVY_VIDEO_EFFECT_BLACK_MAGIC(3),         //3 黑魔法
        MVY_VIDEO_EFFECT_VIRTUAL_MIRROR(4),      //4 虚拟镜像
        MVY_VIDEO_EFFECT_FLUORESCENCE(5),        //5 萤火
        MVY_VIDEO_EFFECT_TIME_TUNNEL(6),         //6 时光隧道
        MVY_VIDEO_EFFECT_DYSPHORIA(7),           //7 躁动
        MVY_VIDEO_EFFECT_FINAL_ZELIG(8),         //8 终极变色
        MVY_VIDEO_EFFECT_SPLIT_SCREEN(9),        //9 分屏
        MVY_VIDEO_EFFECT_HALLUCINATION(10),       //10 幻觉
        MVY_VIDEO_EFFECT_SEVENTYS(11),            //11 70s
        MVY_VIDEO_EFFECT_ROLL_UP(12),             //12 炫酷转动
        MVY_VIDEO_EFFECT_FOUR_SCREEN(13),         //13 四分屏
        MVY_VIDEO_EFFECT_THREE_SCREEN(14),        //14 三分屏
        MVY_VIDEO_EFFECT_BLACK_WHITE_TWINKLE(15),; //15 黑白闪烁

        private int value;
        private MVY_VIDEO_EFFECT_TYPE(int value) {
            this.value = value;
        }
    ...
    private MVY_VIDEO_EFFECT_TYPE type;

    private HashMap<Integer, MVYMagicShader> shortVideoMap;
    private MVYMagicShader shortVideo;

    public MVYGPUImageShortVideoFilter() {
        super();

        shortVideoMap = new HashMap();
    }

    public void setType(MVY_VIDEO_EFFECT_TYPE type) {
        this.type = type;
    }
    ...
    protected void renderToTexture(Buffer vertices, Buffer textureCoordinates) {
        MVYGPUImageEGLContext.syncRunOnRenderThread(new Runnable() {
            ...
            public void run() {
                ...
                if (shortVideo == null) {
                    shortVideo = new MVYMagicShader(type.getValue());
    ...
    public void destroy() {
        ...
        MVYGPUImageEGLContext.syncRunOnRenderThread(new Runnable() {
            @Override
            public void run() {
                for (MVYMagicShader shortVideo : shortVideoMap.values() ) {
